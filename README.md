...
### 🔑 First Boot / Access Preparation

This repository starts **after** Raspberry Pi OS has already been flashed and the nodes are reachable over SSH.
The following steps are therefore still manual prerequisites:
...
```bash
ssh-keygen -t ed25519 -a 100 -f ~/.ssh/id_ed25519 -C "kub62-ansible"
```

If you do **not** inject the public key during imaging, copy it to each node before running Ansible:

```bash
ssh-copy-id -i ~/.ssh/id_ed25519.pub pi@192.168.10.11
```

### 🔐 age / SOPS Prerequisites

The playbook can create the in-cluster `sops-age` secret from an existing age private key, but it does **not**
generate that key for you.

Install age on your control machine first:
...
Create a key pair for SOPS/Flux workflows:
...
If you use [`.sops.yaml`](.sops.yaml), make sure its recipient list contains the public key generated above.
Then point `sops_key` in your inventory to the private key file path (for example `age.agekey`).
...
state actually drifted will make changes. All plays are tagged so you can target individual
layers without touching the rest of the cluster.
...
ansible-playbook playbooks/kub62.yaml -v

# Or provision specific groups / tags:

# 🖥️ Control Plane(s) Only (includes Cilium, SOPS & Flux)
ansible-playbook -l control_plane --tags control-plane,addons playbooks/kub62.yaml -v

# 🧑‍💻 Worker Node(s) Only
ansible-playbook -l worker --tags worker playbooks/kub62.yaml -v

# 🖥️ Kiosk Node Setup
ansible-playbook -l kiosk --tags kiosk playbooks/kub62.yaml -v
...
| Tag              | Scope                                                  |
|------------------|--------------------------------------------------------|
| `ssh`            | SSH host-key trust (runs `always`)                     |
| `base`           | Base OS setup on all nodes                             |
| `k3s`            | k3s install (control-plane + workers)                  |
| `control-plane`  | Control-plane k3s setup                                |
| `addons`         | All control-plane add-ons                              |
...
After a successful run, you can watch the cluster come up either **on a control-plane node** or **from another
machine** after copying the kubeconfig.

### On a control-plane node

The playbook already copies kubeconfig to `/home/{{ ansible_user }}/.kube/config` and exports `KUBECONFIG` in the
user's shell profile, so you can log into a control-plane node and run:
...
Or, when using FluxCD:

```bash
watch flux get all
```

`kubectl` is enough for basic cluster verification. The `flux` command is only useful when Flux is enabled and its
controllers have already been installed in the cluster.

### From your local machine

Copy the kubeconfig from the first control-plane node:

```bash
scp pi@192.168.10.11:/home/pi/.kube/config ~/.kube/kub62-config
```

Then edit `~/.kube/kub62-config` so the `server:` field points to a control-plane IP or DNS name reachable from your
machine (instead of `127.0.0.1`, if necessary), and use it explicitly - e.g.:

```bash
KUBECONFIG=~/.kube/kub62-config kubectl get nodes
KUBECONFIG=~/.kube/kub62-config kubectl get pods --all-namespaces -w
```

If you also want to inspect Flux locally, install the Flux CLI on your workstation first:

```bash
curl -s https://fluxcd.io/install.sh | sudo bash
KUBECONFIG=~/.kube/kub62-config flux get all
```

This is optional and only makes sense if `flux_enabled: true` was used for the cluster. For other platforms, see the
official installation guide: <https://fluxcd.io/flux/installation/>
